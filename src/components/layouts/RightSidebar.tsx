...
import { MagnifyingGlass, TrendUp, Newspaper, UserPlus } from "phosphor-react";
...
export function RightSidebar() {
  return (
    <div className="flex flex-col gap-6 py-8 h-fit sticky top-0">
      {/* Search Bar */}
      <div className="relative group">
        <MagnifyingGlass 
          size={18} 
          className="absolute left-3 top-1/2 -translate-y-1/2 text-slate transition-colors group-focus-within:text-teal" 
        />
        <input 
          type="text" 
          placeholder="Search Agora..." 
          className="w-full bg-paper-dark/30 hover:bg-paper-dark/50 focus:bg-surface border-none rounded-2xl py-2.5 pl-10 pr-4 text-sm font-sans transition-all focus:ring-2 focus:ring-teal/20 outline-none"
        />
      </div>
